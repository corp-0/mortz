...
public class InputPacketTests
{
    ...
    public void RoundTrips_ConsecutiveInputs()
    {
        ...
        Assert.Equal(15, InputPacket.Encode(history.Newest(4)).Length); // generation + 1-byte seq + count + inputs
    }
    ...
    {
        byte[] zeroCount = [0, 0, 0, 0, 0, 0];
        Assert.False(InputPacket.TryDecode(zeroCount, out _));

        byte[] excessive = new byte[6 + (NetConfig.INPUT_REDUNDANCY + 1) * 3];
        excessive[5] = NetConfig.INPUT_REDUNDANCY + 1;
