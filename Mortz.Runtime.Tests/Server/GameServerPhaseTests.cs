...
public class GameServerPhaseTests : IDisposable
{
    ...
    public void InitialAndLiveSnapshotsUseTheSamePresentationProjection()
    {
        ...
        // The readiness gate has not delivered the roster when this is consumed.
        MatchSnapshot initial = MatchSnapshot.Deserialize(load.InitialSnapshot);
        Assert.Equal([7, 8], initial.Players.Select(player => player.Simulation.PeerId));
