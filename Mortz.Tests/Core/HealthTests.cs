...
public class HealthTests
{
    ...
    public void GrazeChipsHealth_DamageSticks_SecondGrazeKills_CorpseIsUntouchable()
    {
        ...
        int expected = BlastSim.Damage(victimBefore, new Vec2(ex, ey));
    ...
    public void DeadState_FreezesMovementAndWeapon()
    {
        ...
        PlayerState after = PlayerSim.Tick(dead,
            new PlayerInput(InputButtons.Right | InputButtons.Jump), TestWorlds.Flat());
        Assert.Equal(dead, after);

        bool fired = WeaponSim.Tick(ref dead, new PlayerInput(InputButtons.Fire), InputButtons.None);
        Assert.False(fired);
