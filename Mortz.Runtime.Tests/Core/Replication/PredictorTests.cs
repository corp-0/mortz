...
public class PredictorTests
{
    ...
    public void ModifierChangesApplyAtTheirEffectiveTickDuringPredictionAndReplay()
    {
        ...
        server.AddPlayer(1);
        Predictor predictor = new(server.Terrain, TestWorlds.NoSpawnProtectionConfig);
        PlayerState initial = server.Players[1];
        predictor.Reconcile(initial, -1, 0);
        StatsModifier boost = new(ModifierId.SPECIAL, StatChange.Mul(Stat.AIR_ACCEL, 2));
        predictor.SetModifiers([boost], effectiveTick: 4, revision: 1);
        PlayerInput input = new(InputButtons.RIGHT);
        for (int tick = 1; tick <= 8; tick++)
        {
            if (tick == 4)
                server.AddModifier(1, boost);
            server.EnqueueInput(1, tick - 1, input);
            server.Step();
            predictor.LocalTick(input);
            Assert.Equal(server.Players[1].Position, predictor.State.Position);
        }
        predictor.Reconcile(initial, -1, 1);
        // The snapshot is tick 1, so replaying its unacked inputs uses that tick as its origin.
        Predictor reference = new(server.Terrain, TestWorlds.NoSpawnProtectionConfig);
        reference.Reconcile(initial, -1, 1);
        reference.SetModifiers([boost], effectiveTick: 4, revision: 1);
        for (int tick = 0; tick < 8; tick++)
        {
            reference.LocalTick(input);
        }
        Assert.Equal(reference.State.Position, predictor.State.Position);
    }
    ...
    public void PredictionStaysTight_ThroughQuantizedWireFormat()
    {
        ...
        for (int t = 0; t < 300; t++)
        {
            ...
            PlayerState wireState = SnapshotWire.Deserialize(server.TakeSnapshot().Serialize()).Players[0];
    ...
    public void SnapshotRoundTrip_PreservesAuthoritativePrevButtons()
    {
        ...
        PlayerState restored = SnapshotWire.Deserialize(new Snapshot(1, [player], []).Serialize()).Players[0];
