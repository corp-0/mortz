...
public class ClientPlayers : IDisposable,
    ...
    IHandle<PlayerModifiersMsg>
{
    ...
    public void OpenMatch(MatchConfig config)
    {
        ...
        ObjectDisposedException.ThrowIf(_disposed, this);
        _matchConfig = config.ToSnapshot().ToMutable();
        _matchConfig.Clamp();
        _matchBaseStats = PlayerStats.Resolve(_matchConfig);
    ...
    {
        _matchConfig = null;
        _matchBaseStats = null;
        Table = RosterSnapshot.Empty;
        RosterRevision = 0;
        foreach (ClientPlayer player in _players.Values)
        {
            player.CloseMatch();
    ...
    {
        if (message.Revision < RosterRevision)
            return;
    ...
    public void Handle(in PlayerModifiersMsg message)
    {
        ...
        if (player.Match == null)
            return;
        if (!player.Match.ApplyModifiers(stats, modifiers, message.Revision, message.EffectiveTick))
            return;
