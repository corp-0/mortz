...
{
    private const string ATTRIBUTE_NAME = "Mortz.Protocol.Net.NetMessageAttribute";
    private const string ROW_ATTRIBUTE_NAME = "Mortz.Protocol.Net.NetRowAttribute";
    ...
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterSourceOutput(messages.Combine(rows), static (spc, models) =>
    ...
    private static string EmitRow(RowModel m)
    {
        ...
        sb.AppendLine("using Mortz.Protocol.Net;");
        sb.AppendLine();
        sb.AppendLine($"namespace {m.Namespace};");
        sb.AppendLine();
        sb.AppendLine($"partial record struct {m.Name} : global::Mortz.Protocol.Net.INetRow<{m.Name}>");
        sb.AppendLine("{");
    ...
    private static void Emit(
        ...
        ImmutableArray<MessageModel> models,
        ImmutableArray<RowModel> rows)
    {
        foreach (Diagnostic d in models.SelectMany(model => model.Diagnostics))
        {
            spc.ReportDiagnostic(d);
        ...
        MessageModel[] valid = models
            ...
            .ToArray();
        RowModel[] validRows = rows
            .Where(r => r.Diagnostics.IsEmpty)
            .OrderBy(r => $"{r.Namespace}.{r.Name}", StringComparer.Ordinal)
            .ToArray();
        if (valid.Length == 0)
            return;
        ...
        spc.AddSource("NetRegistry.g.cs", SourceText.From(EmitRegistry(valid, validRows), Encoding.UTF8));
    ...
    private static string EmitRouter(MessageModel[] models)
    {
        ...
        sb.AppendLine("#nullable enable");
        sb.AppendLine();
        sb.AppendLine("namespace Mortz.Protocol.Net;");
        sb.AppendLine();
        ...
        sb.AppendLine("{");
        sb.AppendLine("    public int? MatchGeneration { get; set; }");
        ...
        {
            string fqn = $"global::{m.Namespace}.{m.Name}";
            sb.AppendLine($"        if (handler is IHandle<TSender, {fqn}> && {HandlerField(m)}.Count > 0)");
            sb.AppendLine($"            throw new global::System.InvalidOperationException(\"{m.Name} already has an authoritative handler.\");");
        }
        foreach (MessageModel m in inbound)
        {
            string fqn = $"global::{m.Namespace}.{m.Name}";
        ...
        sb.AppendLine("    {");
        sb.AppendLine("        if (payload.Length > global::Mortz.Protocol.Net.NetConfig.MAX_ENVELOPE_BYTES)");
        ...
        foreach (MessageModel m in inbound)
        {
            ...
            sb.AppendLine("                    return false;");
            if (m.Fields.Any(member => member.Name == "MatchGeneration"))
            {
                sb.AppendLine("                if (MatchGeneration is int generation && message.MatchGeneration != generation)");
                sb.AppendLine("                    return false;");
            }
    ...
    private static string EmitClientRouter(MessageModel[] models)
    {
        ...
        foreach (MessageModel m in inbound)
        {
            ...
            sb.AppendLine($"                if (!TryDecode<{fqn}>(payload, {fqn}.Deserialize, out {fqn} message))");
            sb.AppendLine("                    return false;");
            if (m.Fields.Any(field => field.Name == "MatchGeneration"))
            {
                sb.AppendLine("                if (MatchGeneration is int generation && message.MatchGeneration != generation)");
                sb.AppendLine("                    return false;");
            }
    ...
    private static string EmitMessage(MessageModel m)
    {
        ...
        sb.AppendLine($"namespace {m.Namespace};");
        sb.AppendLine();
        sb.AppendLine($"partial record struct {m.Name} : global::Mortz.Protocol.Net.INetMessage<{m.Name}>");
        sb.AppendLine("{");
        if (m.Fields.Any(field => field.Name == "MatchGeneration"))
        {
            sb.AppendLine($"    public static {m.Name} InMatch(in {m.Name} message, int generation) => message with {{ MatchGeneration = generation }};");
        }
    ...
    private static string ReadCall(FieldModel f)
    {
        ...
        string read = f.WireType switch
        {
            ...
            "ulong" => "r.ReadUInt64()",
            "float" => "r.ReadSingle()",
            "string" => "global::Mortz.Protocol.Net.NetIo.ReadString(r)",
    ...
    private static string EmitRegistry(MessageModel[] models, RowModel[] rows)
    {
        var sb = new StringBuilder();
        sb.AppendLine("// <auto-generated by Mortz.Net.Gen/>");
        sb.AppendLine("#nullable enable");
        ...
        sb.AppendLine("{");
        sb.AppendLine("    /// <summary>FNV-1a over every message and row shape; carried in Hello so any");
        sb.AppendLine("    /// wire-incompatible build is rejected at connect, no manual bump needed.</summary>");
        sb.AppendLine($"    public const ulong SCHEMA_HASH = 0x{SchemaHash(models, rows):X16}UL;");
        sb.AppendLine();
    ...
    private static ulong SchemaHash(MessageModel[] models, RowModel[] rows)
    {
        ...
        {
            string signature =
                $"{r.Namespace}.{r.Name}({string.Join(",", r.Fields.Select(Signature))})";
            Hash(signature);
        }
        return hash;

        void Hash(string signature)
        {
