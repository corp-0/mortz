...
public sealed class NetMessageGenerator : IIncrementalGenerator
{
    ...
    private static string EmitMessage(MessageModel m)
    {
        ...
        if (toClient)
        {
            ...
            sb.AppendLine();
            sb.AppendLine($"    public void SendTo(int peerId) => NetTransport.Send(NetRegistry.ID_{m.Name}, Serialize(in this), peerId, NetChannel.{channel});");
        }
        else
        {
            sb.AppendLine($"    /// <summary>Raised on the server when this message arrives; sender is the validated peer id.</summary>");
            sb.AppendLine($"    public static event global::System.Action<int, {m.Name}>? Received;");
            sb.AppendLine();
        ...
        sb.AppendLine($"    internal static void Raise(int sender, in {m.Name} m) => {invoke};");
        sb.AppendLine("}");
        return sb.ToString();
    }
    ...
    private static string EmitRegistry(MessageModel[] models)
    {
        ...
        sb.AppendLine();
        sb.AppendLine("    /// <summary>False = malformed, unknown id, or wrong direction for this side.");
        sb.AppendLine("    /// Only decoding is caught here, what a subscriber throws travels on out.</summary>");
        sb.AppendLine("    public static bool Dispatch(ushort msgId, int sender, byte[] payload, bool isServer)");
        sb.AppendLine("    {");
