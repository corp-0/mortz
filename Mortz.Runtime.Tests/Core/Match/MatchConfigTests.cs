...
public class MatchConfigTests
{
    ...
    public void GeneratedMetadata_CoversEveryWritableProperty_InDeclarationOrder()
    {
        ...
        AssertMetadataCoversWritableProperties(
            typeof(ScoreTargetRules), ScoreTargetRulesUiMetadata.Categories);
        AssertMetadataCoversWritableProperties(
            typeof(ScoreLeadRules), ScoreLeadRulesUiMetadata.Categories);
    ...
    public void GeneratedMetadata_BindsTypedAndUntypedValues()
    {
        ...
        ScoreTargetRules rules = new();
        IUiPropertyDescriptor target = ScoreTargetRulesUiMetadata.Categories
            .SelectMany(category => category.Properties)
    ...
    public void WireBlob_CarriesEveryConfigProperty()
    {
        ...
        MatchConfig actual = MatchConfigCodec.FromBytes(expected.ToBytes());
    ...
    private static void AssertWritablePropertiesEqual<T>(T expected, T actual)
    {
        foreach (PropertyInfo property in WritableProperties(typeof(T)))
        {
            ...
            if (expectedValue is EndConditionRules expectedVictory &&
                actualValue is EndConditionRules actualVictory)
            {
    ...
    public void SnapshotEqualityIncludesVictorySubtypeAndValues()
    {
        ...
        {
            Rules = new ModeRules { EndCondition = new ScoreTargetRules { Target = 5 } },
        }.ToSnapshot();
        ...
        {
            Rules = new ModeRules { EndCondition = new ScoreTargetRules { Target = 6 } },
        }.ToSnapshot();
        MatchConfigSnapshot differentSubtype = new MatchConfig
        {
            Rules = new ModeRules { EndCondition = new ScoreLeadRules { Target = 5 } },
        }.ToSnapshot();
    ...
    public void SnapshotCopySharesNoMutableNestedObjects()
    {
        ...
        {
            Rules = new ModeRules { EndCondition = new ScoreLeadRules { Target = 7 } },
        };
        ...
        Assert.NotSame(original.Rules.EndCondition, copy.Rules.EndCondition);
    ...
    [Fact]
    public void SnapshotWireProjectionMatchesTheComposedModeGoldenLayout()
    {
    ...
    public void WireBlob_RejectsTrailingBytes()
    {
        ...
        Assert.Throws<InvalidDataException>(() => MatchConfigCodec.FromBytes([.. valid, 0]));
    }
    ...
    public void FromBytes_ClampsHostileValues()
    {
        ...
        MatchConfig got = MatchConfigCodec.FromBytes(hostile.ToBytes());
    ...
    public void ModeFields_RoundTripTheWire_AndClampHostileValues()
    {
        ...
        Assert.Equal(5, Assert.IsType<ScoreTargetRules>(got.Rules.EndCondition).Target);
        ...
        MatchConfig hostile = new()
        {
            Rules = new ModeRules
            {
                EndCondition = new ScoreLeadRules { Target = 1000 },
            },
        };
        got = MatchConfigCodec.FromBytes(hostile.ToBytes());
        Assert.Equal(999, Assert.IsType<ScoreLeadRules>(got.Rules.EndCondition).Target);
    }
    ...
    public void Clamp_IncludesTheSelectedVictoryRules()
    {
        MatchConfig config = new()
        {
            Rules = new ModeRules
            {
                EndCondition = new ScoreTargetRules { Target = 1000 },
            },
        };

        config.Clamp();

        Assert.Equal(999, Assert.IsType<ScoreTargetRules>(config.Rules.EndCondition).Target);
    }
    ...
    {
        ModeRules rules = new();
        ScoreTargetRules kills = new();
        ScoreLeadRules lead = new();
        Physics physics = new();
