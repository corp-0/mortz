...
public class NetRouterTests
{
    ...
    [Fact]
    public void CommandsRejectASecondAuthoritativeHandler()
    {
        List<string> log = [];
        NetRouter<int> router = new();
        router.Add(new SecondChatHandler(log));
        Assert.Throws<InvalidOperationException>(() => router.Add(new FirstChatHandler(log)));

        Assert.True(router.Dispatch(
            ChatSendMsg.MsgId, 7, ChatSendMsg.Serialize(new ChatSendMsg("hi"))));

        Assert.Equal(["second:7:hi"], log);
    }
    ...
    public void DescribeNamesEveryRoutedMessageAndItsHandlers()
    {
        ...
        Assert.Equal(
            "router: ChatSendMsg -> FirstChatHandler; SetReadyMsg -> ReadyHandler",
            router.Describe());
    }
