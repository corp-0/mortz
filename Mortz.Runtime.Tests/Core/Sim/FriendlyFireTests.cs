...
public class FriendlyFireTests
{
    ...
    public void Team_RidesTheSnapshot_AndSurvivesRespawn()
    {
        ...
        Snapshot restored = SnapshotWire.Deserialize(w.TakeSnapshot().Serialize());
    ...
    private static void FireTheGrazeShot(SimWorld w, ref int seq)
    {
        ...
        (int ex, int ey, _, _, _, _) = Assert.Single(w.Explosions);
        Assert.True(BlastSim.Damage(victimBefore, new Vec2(ex, ey), w.Config.Combat.ToMutable()) > 0,
