...
public sealed class ReplayHistory
{
    ...
    private readonly List<PresentedMatchFrame> _frames = [];

    public void Add(PresentedMatchFrame frame)
    {
        _frames.Add(frame);
    ...
    public ReplayClip? Capture(int eventTick)
    {
        ...
        PresentedMatchFrame[] frames = _frames.GetRange(first, last - first + 1).ToArray();
