...
public class BandwidthTests
{
    ...
    public void RecipientMatchSnapshot_UsesCompactRemoteRecords()
    {
        ...
        RosterSnapshot roster = new([.. simulation.Players
            .Select(p => new RosterEntry(p.PeerId, $"Player {p.PeerId}",
                0, p.Team, (byte)p.PeerId))
        ]);
        MatchSnapshot restored = MatchSnapshot.Deserialize(data, roster);

        Assert.Equal(172, data.Length); // one local record, seven compact remote records
        Assert.Equal(309, fullData.Length); // eight full records
        Assert.Equal(NetConfig.MAX_PLAYERS, restored.Players.Length);
        // Static identity values come from RosterMsg on the slot-id path.
        Assert.Equal(0, restored.Players[0].Skin);
        ...
        Assert.Equal(0, restored.Players[1].Skin);
    ...
    public void SnapshotMortarCount_DoesNotWrapAtByteBoundary()
    {
        ...
        Snapshot restored = SnapshotWire.Deserialize(new Snapshot(9, [], mortars).Serialize());
    ...
    public void HybridTerrainSync_ChoosesSmallCarveLogAndRebuildsExactMask()
    {
        ...
        Assert.Equal(TerrainSyncEncoding.CARVE_LOG, payload.Encoding);
    ...
    public void HybridTerrainSync_SwitchesToBitmapForLongScatteredHistory()
    {
        ...
        Assert.Equal(TerrainSyncEncoding.REMOVED_BITMAP, payload.Encoding);
        Assert.Equal(authoritative.SerializeRemoved(), replica.SerializeRemoved());
    }
}
