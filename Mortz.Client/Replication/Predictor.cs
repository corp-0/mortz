...
public sealed class Predictor
{
    ...
    private readonly List<(int Tick, int Revision, IReadOnlyList<StatsModifier> Modifiers)> _modifierHistory = [];
    private int _modifierRevision = -1;
    private int _appliedModifierRevision = -1;
    ...
    {
        _terrain = terrain;
        _cfg = config.ToSnapshot().ToMutable();
        _cfg.Clamp();
        _zones = zones ?? MapZones.None;
        _tier1 = PlayerStats.Resolve(_cfg);
        _effective = _tier1;
    }

    public void SetModifiers(IReadOnlyList<StatsModifier> modifiers) =>
        SetModifiers(modifiers, int.MinValue, _modifierRevision + 1);

    public void SetModifiers(IReadOnlyList<StatsModifier> modifiers, int effectiveTick, int revision)
    {
        if (revision <= _modifierRevision)
            return;
        _modifierRevision = revision;
        _modifierHistory.Add((effectiveTick, revision, modifiers.ToArray()));
        ApplyModifiersAt(_predictionTick);
    }
    ...
    private void ApplyModifiersAt(int tick)
    {
        ...
        if (selected.Revision == _appliedModifierRevision)
            return;
        _appliedModifierRevision = selected.Revision;
        _modifiers = selected.Modifiers;
        _tier1 = StatsPipeline.Resolve(_cfg, _modifiers);
    ...
    public Vec2 Reconcile(PlayerState serverState, int lastAppliedSeq, int serverTick = -1)
    {
        ...
        Initialized = true;
        _state = state;
        _predictionTick = replayTick;
        if (serverTick >= 0)
        {
            int baseline = _modifierHistory.FindLastIndex(change => change.Tick <= serverTick);
            if (baseline > 0)
                _modifierHistory.RemoveRange(0, baseline);
        }
